    ...
    context 'authorized' do
      ...
      %w(email id created_at updated_at admin).each do |attr|
        it "contains #{attr}" do
          expect(
            response.body
          ).to be_json_eql(me.send(attr.to_sym).to_json).at_path("#{attr}")
        end
      end

      %w(password password_confirmation encrypted_password).each do |attr|
      ...
      %w(email id created_at updated_at admin).each do |attr|
        it "contains #{attr}" do
          users.each_index do |index|
            expect(
              response.body
            ).to be_json_eql(users[index].send(attr.to_sym).to_json).at_path("#{index}/#{attr}")
          end
        end
      end
